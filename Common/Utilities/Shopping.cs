...
namespace Common
{
    public static partial class GlobalUtilities
    {
        ...
        public static string GetProductImagePath(string productImage)
        {
            ...
            if (productImage.IsNullOrEmpty() || productImage.Contains("nopic.gif"))
            {
            ...
            {
                if (productImage.ToUpper().StartsWith("HTTP")) return productImage;
                else return "{0}{1}".FormatWith(GetProductImagePath(), productImage);
            }
        }
        /// <summary>
        /// Gets the tracking url depending on ship method ID
        /// </summary>
        /// <param name="shipmethodid">ID number of ship method/param>
        /// <returns>Url for tracking</returns>
        public static string GetTrackingWebsite(int shipMethodID)
        {
            switch (shipMethodID)
            {
                case 5: return "https://www.fedex.com/apps/fedextrack/?action=track&trackingnumber=";
                case 2: return "https://www.fedex.com/apps/fedextrack/?action=track&trackingnumber=";
                case 4: return "https://www.fedex.com/apps/fedextrack/?action=track&trackingnumber=";
                case 3: return "https://www.fedex.com/apps/fedextrack/?action=track&trackingnumber=";
                default: return "https://www.fedex.com/apps/fedextrack/?action=track&trackingnumber=";
            }
        }
        /// <summary>
        /// Gets the base product image path.
        /// </summary>
        /// <returns>The base absolute Uri used for Exigo product images</returns>
        public static string GetProductImagePath()
        {
            var scheme = "http";
            // When HttpContext.Current is null, we have this here to keep the call from failing - Mike M.
            //try
            //{
            ...
            var cname = "api";
            var sandboxID = Common.GlobalSettings.Exigo.Api.SandboxID;
            if (sandboxID > 0)
                cname = "sandboxapi" + sandboxID;


            return "{0}://{1}.exigo.com/4.0/{2}/productimages/".FormatWith(scheme, cname, GlobalSettings.Exigo.Api.CompanyKey);
        }

        #region Partner Store Integration
        /// <summary>
        ...
        /// <summary>
        /// We use this to check if a customer has purchased a First Order Pack and if they have not, we check their start date to ensure that they 
        /// are in the 30 day window to be able to purchase it in the shopping flow - Mike M.
        /// </summary>
        /// <param name="customerID"></param>
        /// <returns></returns>
        public static bool CanPurchaseFirstOrderPack(int customerID, string firstOrderItemCode)
        {
            var canPurchase = false;

            // First check and see if there is a logged in user, if not and the customerID is 0, we need to return false.
            if (customerID == 0)
            {
                return canPurchase;
            }
            ...
            {
                // Here we check the 30 days from register logic to determine if they can even purchase this item any more
                var startDate = context.Query<DateTime>(@" select CreatedDate from Customers where CustomerID = @customerID
                ...
                var beginningOfMonth = DateTimeExtensions.ToCST(DateTime.Today).BeginningOfMonth().Date;
                var endOfMonth = DateTimeExtensions.ToCST(DateTime.Today).EndOfMonth().AddHours(1);
                ...
                //    {
                //        Body = "start date: {0} <br /> beginning: {1}<br /> end: {2}".FormatWith(startDate.ToString(), beginningOfMonth.ToString(), endOfMonth.ToString()),
                //        From = GlobalSettings.Company.Email,
                //        IsHtml = true,
                //        SMTPConfiguration = GlobalSettings.Emails.SMTPConfigurations.Default,
                //        Subject = "test",
                //        To = new List<string> { "mikeb@exigo.com" }.ToArray(),
                //        ReplyTo = new List<string> { GlobalSettings.Emails.NoReplyEmail }.ToArray()
                //    });
                ...
                // If they are still in the current month window, then we need to run a check using OData to pull the order detail data to make sure they have not bought it yet
                if ((startDate > beginningOfMonth && startDate.Date < endOfMonth))
                {
                    // This call is using OData because SQL may have not caught up yet if the user has just purchased the First Order Pack
                    var firstOrderPackDetails = Exigo.OData().OrderDetails
                        .Where(c => c.Order.CustomerID == customerID && (c.ItemCode == firstOrderItemCode || (c.BusinessVolume > 0 && c.Order.OrderTypeID != (int)OrderTypes.Import)) && c.Order.OrderStatusID >= (int)OrderStatuses.Accepted);

                    if (firstOrderPackDetails.Count() == 0)
                    {
                        canPurchase = true;
                    }
                }
