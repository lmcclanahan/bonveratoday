...
namespace Backoffice
{
    ...
    public class UserIdentity : IIdentity
    {
        ...
        public bool IsSubscribedMonthly
        {
            get
            {
                var subscriptionID = Common.Subscriptions.BackofficeFeatures;

                var cache = HttpRuntime.Cache;
                var key = string.Format("exigo.customersubscriptions.{0}-{1}", CustomerID, subscriptionID);

                if (cache[key] == null)
                {
                    cache.Insert(
                        key,
                        Exigo.GetCustomerSubscription(CustomerID, subscriptionID),
                        null,
                ...
                return !((CustomerSubscription)cache[key]).IsExpired;
            }
        }
